...
def process_hits(id_saison: int, nom_saison: str) -> pd.DataFrame:
    """
    Fonction qui traite les données des mises en échec d'une saison.  
    Enregistre les données dans la cache en plus de les retourner. 
    ...
    Sortie
        données traitées des mises en échec
    """
    if not os.path.exists(f"./cache/traitees/{nom_saison}"):
        os.makedirs(f"./cache/traitees/{nom_saison}")
    ...
    for id_partie in games.index:
        ...
        for event in events:
            ...
            if event.get("event")=="hit":
                hits.append(event)
    columns = ["event","id","game_id","player_id","team_id","home","x_location","y_location","time","period","hit_type"]
    hits_df = pd.DataFrame(hits)[columns].copy()
    hits_df.rename(columns={"event": "event_type"},inplace=True)
